...
export default function ModalBarberoServicio({ servicio, onClose, onGuardado, usuarioId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error: dbError } = esEdicion
      ? await supabase.from('servicios_barbero').update(payload).eq('id', servicio.id)
      : await supabase.from('servicios_barbero').insert(payload);
        ...
        <div className="flex items-center justify-between p-6 border-b border-zinc-800">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-0.5">
              {esEdicion ? 'Actualiza los datos de tu servicio' : 'Crea un nuevo servicio que ofreces'}
            </p>
          </div>
          <button 
            onClick={onClose} 
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-4 py-3 rounded-xl border border-zinc-700 text-zinc-300 font-medium text-sm hover:bg-zinc-800 transition-all"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-4 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-bold text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
